...
def makeRows(rows, cols, largestElementLength, rowLength, matrixToWorkOn, finalTable, color):
    ...
    for i in range(rows):
        ...
        for j in range(cols):
            # If we are using colors then do the same thing or i == 0 but as without (below)
            if ((color != None) and (j == 0 or i == 0)):#cor 
            ...
            # elif j == 0:
            #      currentEl = " " + "\033[38;2;" + str(255) + ";" + str(255) + ";" + str(255) +"m" + matrixToWorkOn[i][j] + "\033[0m"
            else:
