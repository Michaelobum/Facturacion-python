from clases.proveedor import ProveedorLote
...
def proveedorLote():
    ...
    men = Menu(['1).Ingreso','2).Listado','3).Salir'],'     Menu Proveedor Lote')
    opc = men.mostrarMenu()
    while opc != '3':
        system('cls')
        if opc == '1':
            y = 10000
            print(pos(1,25)+'Ingreso Proveedor Lote')
            print(pos(2,25)+'')
            ...
            cta = input('#Cuenta.....:'+pos(8,25))
            prov = ProveedorLote(ruc,nom,dir,tel,mai,cta)
            arch = Archivo()
            arch.escribir('data/proveedor_lote.txt',prov.mostrarProveedor())
            x = input('Registro Grabado, Precione una tecla para continuar...')

        elif opc == '2':
            print(pos(1,15)+'Listado Proveedor por Lotes')
            arch = Archivo()
            datos = arch.leer('data/proveedor_lote.txt')
            tabla=[]
            tabla.append(['Ruc','Nombre','Direccion','Telefono','Email','#Cuenta'])
